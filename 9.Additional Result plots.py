...
def compute_lines(file_path):
    ...
    # Clean key columns
    for col in ['Commissioning date', 'Decommissioning date', 'Total power', 'Repowered Total Capacity (MW)']:
    ...
    df['Repowered Total Capacity (MW)'] = pd.to_numeric(df['Repowered Total Capacity (MW)'], errors='coerce')
    ...
    for _, row in df.iterrows():
        comm = row['Commissioning date']
        repower_cap = row['Repowered Total Capacity (MW)']
        if pd.isna(comm) or pd.isna(repower_cap) or repower_cap <= 0:
            continue
        if pd.isna(row['Decommissioning date']):
...
files = {
    "Approach 1": r"D:\SET 2023\Thesis Delft\Model\Approach 1 PowerDensity.xlsx",
    "Approach 2": r"D:\SET 2023\Thesis Delft\Model\Repowering_Calculation_Stage_2_int_new.xlsx",
    "Approach 3": r"D:\SET 2023\Thesis Delft\Model\Repowering_Calculation_Stage_2_int_new.xlsx",
    "Approach 4": r"D:\SET 2023\Thesis Delft\Model\Repowering_Calculation_Stage_2_int_new_rounding_and_singlereplacement.xlsx"
}
...
cols_to_clean = ['Country', 'Commissioning date', 'Decommissioning date', 'Repowered Total Capacity (MW)',
...
if 'Repowered Total Capacity (MW)' in repowered_df.columns:
    repowered_df['Repowered Total Capacity (MW)'] = pd.to_numeric(repowered_df['Repowered Total Capacity (MW)'],
                                                                  errors='coerce')
...
repowered_df = repowered_df[repowered_df['Repowered Total Capacity (MW)'] > 0]
...
grouped_rep['Repowered Power Density (MW/km²)'] = (
        grouped_rep['Repowered Total Capacity (MW)'] / (grouped_rep['New_Total_Park_Area (m²)'] / 1e6)
)

# Sort by repowered power density descending
...
# Sorted by repowered power density
